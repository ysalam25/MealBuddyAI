...
const CurrentDiet = ({ navigation , route}) => {
    ...
    const fetchCurrentDiet = async () => {
        setLoading(true); 
                try {
                    ...
                    setCurrentDiet(response.body.diets);
                    return response;
                } catch (error) {
    ...
    return (
        <View style={styles.StyledContainer}>
            <View style={styles.InnerContainer}>
                <Text style={styles.SubTitle}>Current Diet</Text>
                <Text style={styles.sectionTitle}>Your current diet:</Text>
                <Text style={styles.sectionTitle1}>{currentDiet.join(', ')}</Text>
                <Text style={styles.sectionTitle}>To change Diet:</Text>
               <DropDownPicker
                  open={isOpen}
                  multiple={true}
                  setOpen={() => setIsOpen(!isOpen)}
                  value={currentValue}
                  items={diet.map((diet: { label: string, name: string }) => ({ label: diet.name, value: diet.label}))}
